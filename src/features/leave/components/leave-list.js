...
class LeaveList extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            appliedLeaveheaders: [
                ...
                { title: "Action", index: 7, cell: row => this.renderActionCell(row) }
            ],
            userAppliedLeaveheaders: [
                ...
                { title: "Actions", index: 8, cell: row => this.renderActionCell(row) }
            ],
            appliedLeave: [],
            appliedLeaveByUsers: [],
            allappliedleave: [],
    ...
    async postApproveReject(leaveApprovedReject) {
        try {
            let { response } = await leaveApplicationService.ApprovedReject(leaveApprovedReject);
    ...
    updateAppliedLeaveState(leaveId) {
        let appliedLeave = this.state.appliedLeave.filter(al => al.leaveId !== leaveId);
        let appliedLeaveByUsers = this.state.appliedLeaveByUsers.filter(al => al.leaveId !== leaveId);
    ...
    async getAllLeavesForCalendar() {
        try {
            let { response } = await leaveApplicationService.getAllLeavesForCalendar();
    ...
    async setAppliedLeaves(flag) {
        try {
            let { response } = await leaveApplicationService.getAllAppliedLeaves(flag);
    ...
    async getAllAppliedLeavesforAdmin() {
        try {
            let { response } = await leaveApplicationService.getAllAppliedLeavesforAdmin();
                ...
                <div className="container bg-white">
                    {this.state.isLoaded ?
                        ...
                        <div>
                            <div className="row">
                                <div className="col">
                                    <h4 className="project-total mt-2">Leaves &nbsp;
                                        {Auth.get("userRole") !== "support" ?
                                            <span title="apply leave" >
                                                <Link to={'/leave/create'} className="links " style={{
                            ...
                            <div className="row">
                                {this.state.selectedView === "calendarView" ? <div className="col-sm-10"></div> :
                                    <div className="col-sm-10">
                                        <nav>
                                            <div className="nav nav-tabs nav-fill" id="pills-tab" role="tablist">
                                                <a onClick={this.setAppliedLeaves.bind(this, 'applied')} className="nav-link active" id="applied-tab" data-toggle="pill" href="#applied" role="tab" aria-controls="applied" aria-selected="true">Applied Leaves</a>
                                                {Auth.get("userRole") !== "user" ? <a onClick={this.setAppliedLeaves.bind(this, 'pending')} className="nav-link" id="pending-tab" data-toggle="pill" href="#pending" role="tab" aria-controls="pending" aria-selected="false">Pending Leaves</a> : ""}
                                                {Auth.get("userRole") !== "user" ? <a onClick={this.setAppliedLeaves.bind(this, 'all')} className="nav-link" id="all-leaves-tab" data-toggle="pill" href="#all-leaves" role="tabpanel" aria-controls="all-leaves" aria-selected="false">Leaves Applied By Users</a> : ""}
                                                {Auth.get("userRole") === "admin" || Auth.get("userRole") === "support" ? <a onClick={this.getAllAppliedLeavesforAdmin.bind(this)} className="nav-link" id="all-user-leaves-tab" data-toggle="pill" href="#all-user-leaves" role="tabpanel" aria-controls="all-leaves" aria-selected="false">All Leaves Applied By Users</a> : ""}
                                            </div>
                                        </nav>
                                    </div>}
                                <div className="col-sm-2">
                                    <div className="float-right">
                                        <div className="input-group input-group-sm ">
                                            ...
                                            <select style={{ fontSize: "11px" }} className="form-control" onChange={this.onSelectViewChange}
                                ...
                                <div id="largeCalendar">
                                    {this.state.selectedView === "calendarView" ?
                                        ...
                                        <div className="leave-div">
                                            <h3 className="project-title d.inline-block mt-3">Leave Status</h3>
                                            <div className="tab-content leave-table" id="pills-tabContent">
                                                <div className="tab-pane fade show active" id="applied" role="tabpanel" aria-labelledby="applied-tab"> {dataTable}</div>
                                                <div className="tab-pane fade" id="pending" role="tabpanel" aria-labelledby="pending-tab">{dataTable}</div>
                                                <div className="tab-pane fade" id="all-leaves" role="tabpanel" aria-labelledby="all-leaves-tab">{dataTable}</div>
                                                <div className="tab-pane fade" id="all-user-leaves" role="tabpanel" aria-labelledby="all-user-leaves-tab">{dataTable}</div>
                                            </div>
                                        </div>
                                    }
                                </div>
...
//                                     <span className="btn btn-xs btn-outline-danger mr-1" title="Delete" onClick={() => {
...
//                                     <span className="btn btn-xs btn-outline-success mr-1" title="Approve" onClick={() => {
//                                         if (window.confirm('Are you sure you wish to Approve this entry?'))
...
//                                     {/* <span>&nbsp;</span> */}
//                                     {Auth.get("userRole") === "admin" ? 
//                                     <span className="btn btn-xs btn-outline-dark mr-1" title="Reject" onClick={() => {
//                                         if (window.confirm('Are you sure you wish to reject this entry?'))
...
//                                 <div>
//                                     {Auth.get("userRole") !== "support" ? 
//                                     <span title="Details"><Link className="btn btn-xs btn-outline-warning" to={url}>
//                                         <i className="far fa-file" style={{fontSize: "10px"}}></i>
//                                     </Link>
//                                     </span> : ''}
//                                     <span>&nbsp;</span>
//                                     {/* <Link to="#" ><i className="fas fa-trash"></i></Link> */}
//                                     {Auth.get("userRole") !== "support" ? 
//                                     <span className="btn btn-xs btn-outline-danger" title="Delete" onClick={() => {
//                                         if (window.confirm('Are you sure you wish to delete this entry?'))
//                                             this.onDeleteListId(row.leaveId)
//                                     }}><i className="far fa-trash-alt"></i>
//                                     </span> : ''}
...
//                 { title: "Status", accessor: "status", index: 7 },
//                 {
//                     title: "Actions", index: 8, cell: row => {
//                         let url = "/leave-details/" + row.leaveId
//                         return (
//                             <div>
//                                 {Auth.get("userRole") !== "support" ? <Link className="btn btn-xs btn-outline-warning" to={url}>
//                                     <i className="far fa-file"></i>
//                                 </Link> : ''}
//                                 <span>&nbsp;</span>
//                                 {/* <Link to="#" ><i className="fas fa-trash"></i></Link> */}
//                                 {Auth.get("userRole") !== "support" ? <a className="btn btn-xs btn-outline-danger" title="Delete" onClick={() => {
//                                     if (window.confirm('Are you sure you wish to delete this entry?'))
//                                         this.onDeleteListId(row.leaveId)
//                                 }}><i className="far fa-trash-alt "></i>
//                                 </a> : ''}
...
//                                 <span>&nbsp;</span>
//                                 {Auth.get("userRole") === "admin" ? 
//                                     <a className="btn btn-xs btn-outline-success mr-1" title="Approve" onClick={() => {
//                                         if (window.confirm('Are you sure you wish to Approve this entry?'))
//                                             this.onApproveListId(row.leaveId)
//                                     }}><i className="far fa-trash-alt"></i>
//                                     </a>:''}
...
//                                     <span>&nbsp;</span>
//                                     {Auth.get("userRole") === "admin" ? 
//                                     <a className="btn btn-xs btn-outline-dark mr-1" title="Reject" onClick={() => {
//                                         if (window.confirm('Are you sure you wish to Reject this entry?'))
//                                             this.onRejectListId(row.leaveId)
//                                     }}><i className="far fa-trash-alt"></i>
//                                     </a>:''}
...
//     async getAllLeavesForCalendar() {
//         let { response, err } = await leaveApplicationService.getAllLeavesForCalendar();
//         this.setState({
//             isLoaded: true
//         });
...
//         let { response, err } = await leaveApplicationService.getAllAppliedLeaves(flag);
...
//     }
//     async getAllAppliedLeavesforAdmin() {
//         let { response, err } = await leaveApplicationService.getAllAppliedLeavesforAdmin();
...
//                                         <a onClick={this.setAppliedLeaves.bind(this, 'applied')} className="nav-link active" id="applied-tab" data-toggle="pill" href="#applied" role="tab" aria-controls="applied" aria-selected="true">Applied Leaves</a>
//                                         {Auth.get("userRole") !== "user" ? <a onClick={this.setAppliedLeaves.bind(this, 'pending')} className="nav-link" id="pending-tab" data-toggle="pill" href="#pending" role="tab" aria-controls="pending" aria-selected="false">Pending Leaves</a> : ""}
//                                         {Auth.get("userRole") !== "user" ? <a onClick={this.setAppliedLeaves.bind(this, 'all')} className="nav-link" id="all-leaves-tab" data-toggle="pill" href="#all-leaves" role="tab" aria-controls="all-leaves" aria-selected="false">Leaves Applied By Users</a> : ""}
//                                         {Auth.get("userRole") === "admin" || Auth.get("userRole") === "support" ? <a onClick={this.getAllAppliedLeavesforAdmin.bind(this)} className="nav-link" id="all-user-leaves-tab" data-toggle="pill" href="#all-user-leaves" role="tab" aria-controls="all-leaves" aria-selected="false">All Leaves Applied By Users</a> : ""}
//                                     </div>
